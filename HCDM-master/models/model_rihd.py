...
class RIHD(BaseModel):
    ...
    def val_step(self):
        ...
        with torch.no_grad():
            for val_data in tqdm.tqdm(self.val_loader):
                ...
                if self.opt['distributed']:
                    self.output, self.visuals = self._run_restoration(self.netG.module)
                else:
                    self.output, self.visuals = self._run_restoration(self.netG)
    ...
    def _log_model_statistics(self):
        model = self.netG.module if self.opt['distributed'] else self.netG
        total_params = sum(p.numel() for p in model.parameters())
        trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
        self.logger.info("ModelParams total={} trainable={}".format(total_params, trainable_params))

    def _log_runtime_statistics(self):
        ...
        with torch.no_grad():
            starter.record()
            if hasattr(model, 'restoration_dpm'):
                ...
                if self.task in ['inpainting','uncropping']:
                    _ = model.restoration(
                        self.cond_image, y_t=self.cond_image, y_0=self.gt_image, mask=self.mask, sample_num=min(4, self.sample_num)
                    )
                else:
                    _ = model.restoration(self.cond_image, sample_num=min(4, self.sample_num))
            ender.record()
            torch.cuda.synchronize()
        elapsed_ms = starter.elapsed_time(ender)
        peak_mem_mb = torch.cuda.max_memory_allocated() / (1024 ** 2)
        self.logger.info("RuntimeStats sample_num={} latency_ms={:.3f} peak_mem_mb={:.1f}".format(min(4, self.sample_num), elapsed_ms, peak_mem_mb))
