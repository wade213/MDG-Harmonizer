...
**结论**: CDP-Net 与 AFM 是当前主要有效模块，组合后 PSNR 达 36.40；FB-Loss 在当前权重下未带来稳定提升（本文将其作为边界感知损失的探索性尝试）。消融四组均在 35.7-36.4 区间，差异 < 0.8 dB。解冻 decoder 均使指标下降，验证了预训练 U-Net 与旧退化管线的深度耦合。
