...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        self.use_degradation_gate = use_degradation_gate
        ...
        if est_mode == "lightweight":
            self.estimation_net = LightweightFCN(base_channels=est_channels)
        else:
            self.estimation_net = FCN()

        if self.use_degradation_gate:
            self.degradation_gate = nn.Sequential(
    ...
    def forward(self, x, mask, gammas):
        ...
        if self.use_degradation_gate:
            # Region gate helps couple degradation estimation with foreground structure.
            gate = self.degradation_gate(torch.cat([noise_map, mask], dim=1))
            noise_map = noise_map * gate
...
class LightweightFCN(nn.Module):
    def __init__(self, base_channels=16):
        super(LightweightFCN, self).__init__()
        self.fcn = nn.Sequential(
            nn.Conv2d(4, base_channels, 3, padding=1, groups=1),
            nn.ReLU(inplace=True),
