...
class Network(BaseNetwork):
    def __init__(self, unet, beta_schedule, module_name='sr3', loss_weights=None, degradation_quantile=0.75, **kwargs):
        ...
        self.degradation_quantile = degradation_quantile
    ...
    def _build_region_weight(self, mask, y_cond, y_0):
        boundary = (F.max_pool2d(mask, kernel_size=3, stride=1, padding=1) -
                    (-F.max_pool2d(-mask, kernel_size=3, stride=1, padding=1))).clamp(0.0, 1.0)
        degradation_strength = (y_cond - y_0).abs().mean(dim=1, keepdim=True)
        flat = degradation_strength.flatten(1)
        threshold = torch.quantile(flat, self.degradation_quantile, dim=1, keepdim=True).view(-1, 1, 1, 1)
        degradation_region = (degradation_strength > threshold).float()
        region_weight = (
            1.0
            + self.loss_weights["foreground"] * mask
            + self.loss_weights["boundary"] * boundary
            + self.loss_weights["degradation"] * degradation_region
        )
    ...
    def restoration_dpm(
        ...
        """DPM-Solver++ 加速采样（替代 1000 步 DDPM，约 40× 加速）。
        ...
        # 构造 denoise_fn 闭包以适配 dpm_solver_restoration 的接口
        raw_denoise_fn = self.denoise_fn

        def model_fn(x, noise_level, deg_vec=None):
            """桥接：modified UNet 接受 (x, mask, noise_level) 并返回噪声预测。"""
            if mask is not None:
                _, noise_list = raw_denoise_fn(x, mask, noise_level)
            else:
                noise_list = raw_denoise_fn(x, noise_level)
            return noise_list  # dpm_solver 内部会取 [-1]
        ...
        if mask is not None:
            y_out = y_0 * (1. - mask) + mask * y_out

        # 构造 ret_arr：取 sample_num 个 y_out 副本
        ret_arr = y_out
    ...
    def forward(self, y_0, y_cond=None, mask=None, noise=None): #参数顺序，真实图片y_0，合成图片(条件图片y_cond)以及mask
        ...
        loss = 0
        if mask is not None:
            region_weight = self._build_region_weight(mask, y_cond, y_0)
        else:
            region_weight = torch.ones_like(y_0[:, :1, :, :])
        region_weight_resized_list = resize_tensor(region_weight)
        if mask is not None: #如果包含mask，则去噪的时候，将随机噪声y_noisy*mask+真实图片*（1-mask）作为一个输入
            ...
            noise_level_gt = (y_cond - y_0).abs().mean(dim=1, keepdim=True).detach()
            weighted_degradation = region_weight * mask
            loss += self.loss_weights["degradation_map"] * F.mse_loss(
                noise_level * weighted_degradation,
                noise_level_gt * weighted_degradation
            )
            for i in range(len(noise_hat_list)):
                weighted_mask = mask_resized_list[i] * region_weight_resized_list[i]
                loss += self.loss_fn(weighted_mask*noise_resized_list[i], weighted_mask*noise_hat_list[i])
        else:
            noise_hat_list = self.denoise_fn(torch.cat([y_cond, y_noisy], dim=1), sample_gammas)
            for i in range(len(noise_hat_list)):
                loss += self.loss_fn(region_weight_resized_list[i]*noise_resized_list[i], region_weight_resized_list[i]*noise_hat_list[i])
        return loss
